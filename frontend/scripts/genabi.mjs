...
// TODO: Re-enable after deploying updated contract on Sepolia
if (deployLocalhost && deploySepolia) {
  if (
    JSON.stringify(deployLocalhost.abi) !== JSON.stringify(deploySepolia.abi)
  ) {
    console.warn(
      `${line}⚠️  WARNING: Deployments on localhost and Sepolia differ.\nABI compatibility check skipped for development.\nConsider re-deploying the contracts on both networks when ready for production.${line}`
    );
